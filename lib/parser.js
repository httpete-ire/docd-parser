    ...
    while(src) {
      ...
      if((match = this.rules.strong.start.exec(src)) !== null) {
        ...
        if(match && delimiter === match[2]) {
          ...
          // get value inside strong tag
          match = match[1].substring(delimiter.length);

          // if value is empty insert empty text node
          // else parse the inline value for other
          // markdown components
          if(!match) {
            node.addChild(new Node('text', ''));
          } else {
            this._inlineParse(match, node);
          }
